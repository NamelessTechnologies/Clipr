...
function CommentBox(props: { post_id: number, user_id: number, username: string, user_pfp: string }) {
    ...
    const postComment = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const newlyPostedComment: CommentModel = {comment_id: comment_id, parent_id: null, post_id: props.post_id, user_id: props.user_id, username: props.username, pfp_url: props.user_pfp, liked: false, num_likes: 0, content: userComment};
