...
function Comment(props: {
  ...
  return (
    <div
      className={`flex w-full text-sm px-3 ${
        props.isReply ? "pt-2 pb-1" : "pt-3 pb-2"
      }`}
    >
      <img
        src={user_pfp}
        className={`${
          props.isReply ? "w-10 h-10" : "w-12 h-12"
        } rounded-full mr-3`}
      ></img>
      ...
      <div className="flex flex-col w-9/12 mr-2">
        <span
          className={`${
            props.isReply ? "text-replySize" : "text-sm"
          } text-gray-300 cursor-pointer w-fit`}
        >
          {username}
        </span>
        <span
          className={`${
            props.isReply ? "text-replySize mt-1" : "text-sm mt-2"
          } text-white`}
        >
          {content}
        </span>
