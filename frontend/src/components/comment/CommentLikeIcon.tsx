...
function CommentLikeIcon(props: { liked: boolean, num_likes: number }) {
    ...
    return (
        <div className="flex flex-col items-center mt-7">
                {liked ? (
                    <FaHeart className={`text-red-500 text-base mt-0.5 cursor-pointer ${animate ? "enlarge-shrink" : ""}`} onClick={updateLike}/>
                ) : (
                    <FaRegHeart className="text-white text-base mt-0.5 cursor-pointer" onClick={updateLike}/>
                )}
